...
class Action {
	...
	function save_roomschedule() {
		...
		if (empty($id)) {
			...
			if ($query->num_rows > 0) {
				return 3; // Return 3 for duplicate faculty and days
			}
	
			// Check for existing loading entry with the same timeslot and room
			...
			if ($query->num_rows == 0) {
				...
				$subjectQuery = $this->db->query("SELECT * FROM subjects WHERE subject='$subject'");
				foreach ($subjectQuery as $key) {
					$status = $key['status'];
					$newstats = $status - 1;
					$subjectStats = "status =" . $newstats;
				...
				return 2; // Return 2 for duplicate entry based on timeslot and room
			}
		} else {
